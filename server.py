...
log.startLogging(open('/var/log/miguan.log', 'w'))
...
SERVER_RSA_PRIVATE = '/etc/ssh/ssh_host_rsa_key'
SERVER_RSA_PUBLIC = '/etc/ssh/ssh_host_rsa_key.pub'

# Path to RSA SSH keys accepted by the server.
CLIENT_RSA_PUBLIC = '/etc/ssh/client_rsa.pub'
...
class EchoProtocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        session.
        """
        self.transport.write("[user@133-130-101-184 /]#")
        if data == b'\r':
            data = b'\r\n'
        elif data == b'\x03': #^C
...
if __name__ == '__main__':
    endpoints.serverFromString(reactor, "tcp:8089").listen(server.Site(Counter()))
